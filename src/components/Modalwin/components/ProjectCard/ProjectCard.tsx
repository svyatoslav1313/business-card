import './ProjectCard.scss';

type Props = {
  title: string;
  description: string;
  background: string;
  demoLink: string;
  tags: string[];
}

export const ProjectCard: React.FC<Props> = ({
  title,
  description,
  background,
  demoLink,
  tags,
}) => {
  return (
    <div className='projectcard'>
      <div
        className='projectcard__icon'
        style={{ background: background }}
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-code-xml" aria-hidden="true"><path d="m18 16 4-4-4-4"></path><path d="m6 8-4 4 4 4"></path><path d="m14.5 4-5 16"></path></svg>
      </div>
      <h1 className='projectcard__title'>{title}</h1>
      <p className='projectcard__description'>{description}</p>
      <div className='projectcard__tags'>
        {tags.map(tag => (
          <div className="projectcard__tag" key={tag}>{tag}</div>
        ))}
      </div>
      <div className='projectcard__links'>
        <a className="projectcard__link">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-github" aria-hidden="true"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"></path><path d="M9 18c-4.51 2-5-2-7-2"></path></svg>
          Code
        </a>
        <a
          href={demoLink}
          className="projectcard__link"
          target="_blank"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-external-link" aria-hidden="true"><path d="M15 3h6v6"></path><path d="M10 14 21 3"></path><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path></svg>
          Live Demo
        </a>
      </div>
    </div>
  );
}